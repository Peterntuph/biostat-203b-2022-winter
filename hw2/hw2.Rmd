...
group_lab <- labevents_tble_1st_visit %>%
...
#combine with patients.csv.gz (age at admission > 18)
patient18_icu_combine <-
  patients_tble %>% 
...
ad_pa_icu_combine <-
  admissions_tble %>% 
  inner_join(patient18_icu_combine, by = "subject_id")

#first lab measurements during ICU stay
ad_pa_icu_lab_combine <-
  labevents_tble_1st_visit %>% 
  inner_join(ad_pa_icu_combine, by = "subject_id")

#first vital measurements during ICU stay
mimic_icu_cohort <-
  chartevents_tble %>% 
  inner_join(ad_pa_icu_lab_combine, by = "subject_id")

#an indicator variable `thirty_day_mort`
mimic_icu_cohort <- 
  mimic_icu_cohort %>% 
  mutate(
  dod2 = coalesce(admittime,dod),
  day_mort = dod2 - admittime
)
