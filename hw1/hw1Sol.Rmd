...
As we know that csv.gz files are the CSV file compressed with gzip utility. The reason why those data distributed as .csv.gz files are because they are too large. In order to reduce the size of files, they used compressed file type csv.gz
...
#man zmore
'Zmore command is a handy utility that lets you view gzipped files one screen at a time (much like the more command) without first decompressing them.'
#man zgrep
...
#There are 66155 rows in this data file.
```
```{bash, eval=F}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $1}' | sort | uniq | wc -l
#There are 256879 unique patients (identified by `subject_id`) in this data file
```
...
```{bash, eval=F}
#admission_type
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $6}' | sort | uniq -c | wc -l
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $6}' | sort | uniq -c | head -10
#There are 9 unique admission_type value (identified by `admission_type`) in this data file
```
```{bash, eval=F}
#admission_location
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $7}' | sort | uniq -c | wc -l
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $7}' | sort | uniq -c | head -13
#There are 12 unique admission_location value (identified by `admission_location`) in this data file
```
```{bash, eval=F}
#insurance
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $9}' | sort | uniq -c | wc -l
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $9}' | sort | uniq -c | head
#There are 3 unique insurance value (identified by `insurance`) in this data file
```
```{bash, eval=F}
#ethnicity
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $12}' | sort | uniq -c | wc -l
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '{print $12}' | sort | uniq -c | head
#There are 8 unique ethnicity value (identified by `ethnicity`) in this data file
```
