...
-   Are the materials (files and instructions) submitted to the `main` branch sufficient for reproducing all the results? Just click the `knit` button will produce the final `html` on teaching server? 
    ...
    When I press knit, I receive an error at line 233. This is involving your middle.sh file. Loading middle.sh into the hw1 folder and calling it from there should fix the issue. Should be able to knit fully with one click by downloading the repository `-1`.
...
-   [Rule 3.](https://google.github.io/styleguide/Rguide.xml#linelength) The maximum line length is 80 characters. 
    ...
    3.5 and 3.6 both have lines extending past 80 characters. Lines 125:166. `-4`.
