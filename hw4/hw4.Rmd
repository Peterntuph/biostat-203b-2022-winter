...
fit <- randomForest(thirty_day_mort ~ ., data=icu_train
                    , maxnodes=4
                    , ntree=30
                    , norm.votes=FALSE
                    )
...
##
status_train <- as.numeric(icu_train$thirty_day_mort)
pred_train <- as.numeric(predict(model1, icu_train) >0.5)
auc1 <- round(auc(status_train, pred_train),4)
auc2 <- round(auc(status_test, pred),4)


df <- rbind(data.frame(predictor = predict(model1, icu_train),
                       known.truth = icu_train$thirty_day_mort,
                       model = "train"),
            data.frame(predictor = predict(model1, icu_test),
                       known.truth = icu_test$thirty_day_mort,
                       model = "test"))

ggplot(df, aes(d = known.truth, m = predictor, color = model)) +
      geom_roc(n.cuts = 0) +
      ggtitle(paste0('ROC Curve for Logistics Model '
                      , '(Training & Testing AUC = ', auc1 ,' & ', auc2 ,')')) +
      theme_minimal()
```
